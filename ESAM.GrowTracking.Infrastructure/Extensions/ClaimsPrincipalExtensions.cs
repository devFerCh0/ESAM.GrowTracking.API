    ...
    public static class ClaimsPrincipalExtensions
    {
        ...
        public static AccessTokenType? GetAccessTokenType(this ClaimsPrincipal user)
        {
            ...
            if (string.IsNullOrWhiteSpace(raw))
                return null;
            return EnumHelper.IsValidFromString<AccessTokenType>(raw) ? EnumHelper.ParseFromString<AccessTokenType>(raw) : null;
        }
        ...
        public static int? GetWorkProfileSelectedId(this ClaimsPrincipal user)
        {
            ArgumentNullException.ThrowIfNull(user);
            var raw = user.FindFirst(CustomClaimTypes.WorkProfileSelectedId)?.Value;
            return int.TryParse(raw, out var value) ? value : null;
        }
        ...
        public static WorkProfileType? GetWorkProfileType(this ClaimsPrincipal user)
        {
            ...
            if (string.IsNullOrWhiteSpace(raw))
                return null;
            return EnumHelper.IsValidFromString<WorkProfileType>(raw) ? EnumHelper.ParseFromString<WorkProfileType>(raw) : null;
        }

        public static int? GetRoleCampusSelectedId(this ClaimsPrincipal user)
        {
            ArgumentNullException.ThrowIfNull(user);
            var raw = user.FindFirst(CustomClaimTypes.RoleCampusSelectedId)?.Value;
            return int.TryParse(raw, out var value) ? value : null;
        }
